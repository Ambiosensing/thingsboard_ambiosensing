...
def getCustomers(textSearch=None, idOffset=None, textOffset=None, limit=10):
    ...
    if textSearch:
        textSearch = urllib.request.quote(textSearch.encode('UTF-8')).replace('/', '%2F')
        url_strings.append("textSearch=" + str(textSearch))
    if idOffset:
        idOffset = urllib.request.quote(idOffset.encode('UTF-8')).replace('/', '%2F')
        url_strings.append("idOffset=" + str(idOffset))
    if textOffset:
        textOffset = urllib.request.quote(textOffset.encode('UTF-8')).replace('/', '%2F')
