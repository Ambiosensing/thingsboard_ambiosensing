...
def getTenantDevices(type=None, textSearch=None, idOffset=None, textOffset=None, limit=10):
    ...
    # This is a REGULAR only service, i.e., the service call requires an REGULAR type authorization token to be executed. As before, hide this from the user by setting the type of authorization token expected inside the method rather than to
    # hope for the user to know this.
    regular_auth_token = utils.get_auth_token(admin=False)
    ...
    service_dict = utils.build_service_calling_info(regular_auth_token, service_endpoint)
...
def getCustomerDevices(customer_name, type=None, textSearch=None, idOffset=None, textOffset=None, limit=50):
    ...
    sql_select = """SELECT """ + ", ".join(columns_to_retrieve) + """ FROM """ + str(config.mysql_db_tables[module_table_key]) + """ WHERE name LIKE %s;"""
